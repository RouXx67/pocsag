...
install_dependencies() {
    ...
    print_status "Configuration des dépendances Python..."
    # Utilisation de apt pour éviter le problème "externally-managed-environment" de Python 3.13
    apt install -y python3-requests 2>/dev/null || \
        pip3 install --break-system-packages requests 2>/dev/null || \
        pip3 install requests 2>/dev/null || \
        echo -e "⚠️  Installation Python manuelle requise : ${YELLOW}pip3 install requests${NC}"
}
